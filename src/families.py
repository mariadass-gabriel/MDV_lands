...
@dataclass
class AsymGaussianPoss:
    """Split-Gaussian possibility: left/right half-Gaussians with different sigmas."""
    ...
    def log_poss(self, theta_grid: Tensor, params: Dict[str, float] | None = None) -> Tensor:
        mu = params["mu"] if params is not None else self.mu
        sl = params["sigma_l"] if params is not None else self.sigma_l
        sr = params["sigma_r"] if params is not None else self.sigma_r
        log_vals = torch.where(
            theta_grid < mu,
            -0.5 * ((theta_grid - mu) / sl) ** 2,
            -0.5 * ((theta_grid - mu) / sr) ** 2)
        return _normalize_log(log_vals)

    def mode(self, params: Dict[str, float] | None = None) -> float:
        return params["mu"] if params is not None else self.mu

    def fit(self, log_g_star: Tensor, theta_grid: Tensor) -> "AsymGaussianPoss":
        """mu = argmax g*, sigma_l/sigma_r from curvature on each side of the mode."""
        idx_mode = int(log_g_star.argmax().item())
        mu = theta_grid[idx_mode].item()
        dtheta = (theta_grid[-1] - theta_grid[0]).item() / (len(theta_grid) - 1)

        def _sigma_at(i_center: int) -> float:
            i = max(1, min(i_center, len(theta_grid) - 2))
            curv = (log_g_star[i + 1] - 2 * log_g_star[i] + log_g_star[i - 1]).item() / dtheta ** 2
            return 1.0 / math.sqrt(max(-curv, 1e-6))

        return AsymGaussianPoss(
            mu=mu,
            sigma_l=_sigma_at(max(1, idx_mode - 1)),
            sigma_r=_sigma_at(min(len(theta_grid) - 2, idx_mode + 1)))